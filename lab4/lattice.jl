...
struct Neighbour{T1<:Int,T2<:Real}
	atom1::T1
	atom2::T1
	distance::T2
	pos_diff::Vector{T2}
...
struct Neighbour_list{T<:Real}
	cv::rs_vector{T}
	fc_lattice::Matrix{T}
	cut_off::T
end

struct NL_state{T1<:Int, T2<:Real}
	max_index::Vector{T1}
	cut_off_square::T2
	index::Vector{T1}
end

function Base.iterate(nl::Neighbour_list)
	cut_off_square = nl.cut_off*nl.cut_off
	max_i = size(nl.fc_lattice,2)
	max_index = [max_i, max_i]
	index = [1,1]
	state = NL_state(max_index, cut_off_square, index)
	record, state = Base.iterate(nl, state)
	return record, state
end

function Base.iterate(nl::Neighbour_list, state::NL_state)
	max_i = state.max_index[1]
	max_j = state.max_index[2]
	i = state.index[1]
	j = state.index[2]
	while true
		if (i != max_i) & (j==max_j)
		...
		@views diff_3d!(nl.fc_lattice[:,i], nl.fc_lattice[:,j], temp1)
		apply_pbc!(temp1, temp2)
		fc_to_real_space!(nl.cv, temp2, temp1)
		distance_square = dot_product(temp1, temp1)
	
		if distance_square <= state.cut_off_square
			distance = sqrt(distance_square)
			record = Neighbour(i, j, distance,temp1)
			state.index[1] = i
			state.index[2] = j
			return record, state
		end
...
function neighbour_list(
	...
	record_neighbour = Neighbour_list(cv, fc_lattice ,cut_off_distance)
...
function apply_pbc!(x::AbstractVector{T}, result::AbstractVector{T}) where {T<:Real}
